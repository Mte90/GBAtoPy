...
cpsr_n = (result_cmp >> 31) & 1
cpsr_z = 1 if result_cmp == 0 else 0
# Carry: 1 if no borrow (result >= 0 in unsigned)
cpsr_c = 1 if ({rd} >= {rm}) else 0
...
cpsr_n = (result_cmn >> 31) & 1
cpsr_z = 1 if result_cmn == 0 else 0
# Carry: 1 if overflow occurred (sum >= 2^32)
cpsr_c = 1 if ({rd} + {rm}) >= 0x100000000 else 0
...
pub fn generate_tst_instruction(ops: &[String]) -> String {
    ...
    format!(
        r#"# TST {rd}, {rm}
result_tst = {rd} & {rm}
cpsr_n = (result_tst >> 31) & 1
cpsr_z = 1 if result_tst == 0 else 0
cpsr_c = 0  # No carry for AND
cpsr_v = 0  # No overflow for AND"#
    )
}
...
pub fn generate_teq_instruction(ops: &[String]) -> String {
    ...
    format!(
        r#"# TEQ {rd}, {rm}
result_teq = {rd} ^ {rm}
cpsr_n = (result_teq >> 31) & 1
cpsr_z = 1 if result_teq == 0 else 0
cpsr_c = 0  # No carry for EOR
cpsr_v = 0  # No overflow for EOR"#
    )
}
...
{rd} = result_add
cpsr_n = (result_add >> 31) & 1
cpsr_z = 1 if result_add == 0 else 0
cpsr_c = 1 if ({rd} + {rm}) >= 0x100000000 else 0
...
pub fn generate_adc_instruction(ops: &[String]) -> String {
    ...
    format!(
        r#"# ADC {rd}, {rm}
result_adc = ({rd} + {rm} + cpsr_c) & 0xFFFFFFFF
{rd} = result_adc
cpsr_n = (result_adc >> 31) & 1
cpsr_z = 1 if result_adc == 0 else 0
cpsr_c = 1 if ({rd} + {rm} + cpsr_c) >= 0x100000000 else 0
# Signed overflow
rd_signed = {rd} if {rd} < 0x80000000 else {rd} - 0x100000000
rm_signed = {rm} if {rm} < 0x80000000 else {rm} - 0x100000000
...
{rd} = result_sub
cpsr_n = (result_sub >> 31) & 1
cpsr_z = 1 if result_sub == 0 else 0
cpsr_c = 1 if {rd} >= {rm} else 0  # No borrow
# Signed overflow
rd_signed = {rd} if {rd} < 0x80000000 else {rd} - 0x100000000
rm_signed = {rm} if {rm} < 0x80000000 else {rm} - 0x100000000
...
result_sbc = ({rd} - {rm} + cpsr_c - 1) & 0xFFFFFFFF
{rd} = result_sbc
cpsr_n = (result_sbc >> 31) & 1
cpsr_z = 1 if result_sbc == 0 else 0
cpsr_c = 1 if {rd} >= {rm} + (1 - cpsr_c) else 0
# Signed overflow
rd_signed = {rd} if {rd} < 0x80000000 else {rd} - 0x100000000
rm_signed = {rm} if {rm} < 0x80000000 else {rm} - 0x100000000
...
{rd} = result_rsb
cpsr_n = (result_rsb >> 31) & 1
cpsr_z = 1 if result_rsb == 0 else 0
cpsr_c = 1 if {rm} >= {rd} else 0  # No borrow
# Signed overflow
rm_signed = {rm} if {rm} < 0x80000000 else {rm} - 0x100000000
rd_signed = {rd} if {rd} < 0x80000000 else {rd} - 0x100000000
...
pub fn generate_rsc_instruction(ops: &[String]) -> String {
    ...
    format!(
        r#"# RSC {rd}, {rm}
# RSC = Rm - Rd + C - 1
result_rsc = ({rm} - {rd} + cpsr_c - 1) & 0xFFFFFFFF
{rd} = result_rsc
cpsr_n = (result_rsc >> 31) & 1
cpsr_z = 1 if result_rsc == 0 else 0
cpsr_c = 1 if {rm} >= {rd} + (1 - cpsr_c) else 0
# Signed overflow
rm_signed = {rm} if {rm} < 0x80000000 else {rm} - 0x100000000
rd_signed = {rd} if {rd} < 0x80000000 else {rd} - 0x100000000
